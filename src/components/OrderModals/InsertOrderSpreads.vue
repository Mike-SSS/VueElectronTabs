            ...
            <v-row>
              <v-col cols="6"
                ...
                <v-text-field
                  ...
                  class="my-2"
                  label="Strike"
                  v-model.number="form.differential"
                  :rules="[
                    (msg: number) => !!msg || 'Differential Required',
                    (msg: number) => msg >= 0 || 'Differential cannot be less or equal to 0',
                  ]"
                ></v-text-field
              ></v-col>
              <v-col cols="6"
...
const convertItemToForm = (item: MainModel): FormModel => {
  ...
  return {
    ...
    differential: Number(item.openPrice),
    dealer: "BDA",
...
function enterTrade() {
  ...
  console.log(
    `Trade ${form.value.qty} @ ${form.value.differential} for ${props.item.contractDisplay.contractDisplay} `
  );
  const payload: IInsertOrderFutures = {
    ...
    value: Number(form.value.differential),
